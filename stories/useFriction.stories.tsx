...
interface FrictionSVGState {
  outerPath: number;
  innerPath: number;
  circlePath: number;
}

enum FrictionSVGActionTypes {
  UpdateOuterPath,
  UpdateInnerPath,
  UpdateCirclePath,
}

interface FrictionSVGAction {
  type: FrictionSVGActionTypes;
  payload: {
    pathLength: number;
  };
}

const initialState: FrictionSVGState = {
  outerPath: 0,
  innerPath: 0,
  circlePath: 0,
};

const reducer = (state: FrictionSVGState, action: FrictionSVGAction) => {
  switch (action.type) {
    case FrictionSVGActionTypes.UpdateOuterPath:
      return {
        ...state,
        outerPath: action.payload.pathLength,
      };
    case FrictionSVGActionTypes.UpdateInnerPath:
      return {
        ...state,
        innerPath: action.payload.pathLength,
      };
    case FrictionSVGActionTypes.UpdateCirclePath:
      return {
        ...state,
        circlePath: action.payload.pathLength,
      };
  }
};

export const FrictionSVG: React.FC = () => {
  const [state, dispatch] = React.useReducer(reducer, initialState);

  React.useLayoutEffect(() => {
    const outerPath = document.querySelector<SVGPathElement>('#outerPath');
    const innerPath = document.querySelector<SVGPathElement>('#innerPath');
    const circlePath = document.querySelector<SVGPathElement>('#circlePath');
    dispatch({
      type: FrictionSVGActionTypes.UpdateOuterPath,
      payload: { pathLength: outerPath?.getTotalLength() ?? 0 },
    });
    dispatch({
      type: FrictionSVGActionTypes.UpdateInnerPath,
      payload: { pathLength: innerPath?.getTotalLength() ?? 0 },
    });
    dispatch({
      type: FrictionSVGActionTypes.UpdateCirclePath,
      payload: { pathLength: circlePath?.getTotalLength() ?? 0 },
    });
  }, []);
  ...
  const [outerPathProps] = useFriction<SVGPathElement>({
    from: {
      strokeDashoffset: state.outerPath,
    },
    to,
    config,
    infinite: true,
  ...
  const [innerPathProps] = useFriction<SVGPathElement>({
    from: {
      strokeDashoffset: state.innerPath,
    },
    to,
    config,
    infinite: true,
  ...
  const [circlePathProps] = useFriction<SVGPathElement>({
    from: {
      strokeDashoffset: state.circlePath,
    },
    to,
    config,
    infinite: true,
  ...
  return (
    ...
    >
      ...
      <path
        ...
        id="outerPath"
        strokeDasharray={state.outerPath}
        {...outerPathProps}
      />
      <path
        ...
        className="react-logo"
        id="innerPath"
        strokeDasharray={state.innerPath}
        {...innerPathProps}
      />
      <path
        ...
        className="react-logo"
        id="circlePath"
        strokeDasharray={state.circlePath}
        {...circlePathProps}
      />
    </svg>
  );
